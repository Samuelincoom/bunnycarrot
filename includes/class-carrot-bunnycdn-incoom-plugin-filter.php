 ...
 */
abstract class carrot_bunnycdn_incoom_plugin_Filter {

	/**
	 * The key used for storing the URL cache.
	 */
	const CACHE_KEY = 'incoom_carrot_bunnycdn_incoom_plugin_amazonS3_cache';

	/**
	 * The cache group used by an external object cache for posts.
	 */
	const POST_CACHE_GROUP = 'incoom_carrot_bunnycdn_incoom_plugin_post_amazonS3_cache';

	/**
	 * The cache group used by an external object cache for options.
	 */
	const OPTION_CACHE_GROUP = 'incoom_carrot_bunnycdn_incoom_plugin_option_amazonS3_cache';
	...
	protected static $cloud_urls = array();
	protected static $provider_removed_urls = array();
	...
	protected function filter_media_widget( $instance, $update_cache ) {
		...
		foreach ( $instance as $key => $value ) {
			...
			if ( carrot_bunnycdn_incoom_plugin_Utils::is_url( $value ) ) {
	...
	protected function process_content( $content, $cache, &$to_cache ) {
		...
		if(count(self::$provider_removed_urls) > 0){

			$upload_dir = wp_upload_dir();
			foreach(self::$provider_removed_urls as $post_id => $find_url){
				$key = carrot_bunnycdn_incoom_plugin_Utils::get_key_from_url($find_url, false);
				$url_replace = "{$upload_dir['baseurl']}/{$key}";
				if($url_replace){
					$content = str_replace( $find_url, $url_replace, $content );
				}
			}
		}
	...
	public function url_provider_needs_remove( $post_id, $url ) {
	...
	protected function get_urls_from_img_src( $content, &$to_cache ) {
		...
		foreach ( $matches as $image ) {
			...
			if ( $this->url_provider_needs_remove( absint( $class_id[1] ), $url ) ) {
				self::$provider_removed_urls[absint( $class_id[1] )] = $url;
				continue;
			}
			
			if ( ! $this->url_needs_replacing( $url ) ) {
				// URL already correct, skip
				self::$cloud_urls[absint( $class_id[1] )] = $url;
				continue;
			}

			$url = carrot_bunnycdn_incoom_plugin_Utils::reduce_url( $url );
	...
	protected function get_urls_from_content( $content, $cache, &$to_cache ) {
		...
		foreach ( $matches as $url ) {
			...
			$attachment_id = null;
			$bare_url      = carrot_bunnycdn_incoom_plugin_Utils::reduce_url( $url );
	...
	protected function attachment_id_matches_src( $attachment_id, $url ) {
		...
		$base_url = carrot_bunnycdn_incoom_plugin_Utils::reduce_url( $this->get_base_url( $attachment_id ) );
	...
	protected function push_to_url_pairs( &$url_pairs, $attachment_id, $find, &$to_cache ) {
		$find_full = carrot_bunnycdn_incoom_plugin_Utils::remove_size_from_filename( $find );
		$find_full = $this->normalize_find_value( incoom_carrot_bunnycdn_incoom_plugin_maybe_remove_query_string( $find_full ) );
		$find_size = $this->normalize_find_value( incoom_carrot_bunnycdn_incoom_plugin_maybe_remove_query_string( $find ) );
		...
		if ( ! isset( $parts['scheme'] ) ) {
			$replace_full = carrot_bunnycdn_incoom_plugin_Utils::remove_scheme( $replace_full );
			$replace_size = carrot_bunnycdn_incoom_plugin_Utils::remove_scheme( $replace_size );
		}
		...
		$replace_full = incoom_carrot_bunnycdn_incoom_plugin_maybe_remove_query_string( $replace_full );
		$replace_size = incoom_carrot_bunnycdn_incoom_plugin_maybe_remove_query_string( $replace_size );
	...
	protected function get_size_string_from_url( $attachment_id, $url ) {
		...
		$basename = wp_basename( incoom_carrot_bunnycdn_incoom_plugin_maybe_remove_query_string( $url ) );
	...
	protected function url_cache_failure( $url, &$to_cache ) {
		$full    = carrot_bunnycdn_incoom_plugin_Utils::remove_size_from_filename( $url );
		$failure = array(
			'timestamp' => time(),
		);
	...
	protected function replace_urls( $content, $url_pairs ) {
		...
		foreach ( $url_pairs as $find => $replace ) {
			...
			$key = carrot_bunnycdn_incoom_plugin_Utils::get_key_from_url($replace);

			$post_id = carrot_bunnycdn_incoom_plugin_get_post_id($key);
			if($post_id){
				$s3_path = get_post_meta( $post_id, '_wp_incoom_carrot_bunnycdn_s3_path', true);
				if ( $s3_path != '_wp_incoom_carrot_bunnycdn_s3_path_not_in_used' && $s3_path != null ) {
					$enable_webp = carrot_bunnycdn_incoom_plugin_can_use_webp();
					if($enable_webp){
						$new_replace = carrot_bunnycdn_incoom_plugin_Utils::get_webp_url_with_key($post_id, $key);
	...
	public function get_post_cache( $post = null ) {
		...
		$post_id = carrot_bunnycdn_incoom_plugin_Utils::get_post_id( $post );
	...
	protected function set_post_cache( $post, $data ) {
		...
		if ( wp_using_ext_object_cache() ) {
			$expires = apply_filters( 'carrot_bunnycdn_incoom_plugin_' . self::POST_CACHE_GROUP . '_expires', DAY_IN_SECONDS, $post_id, $data );
	...
	protected function set_option_cache( $data ) {
		if ( wp_using_ext_object_cache() ) {
			$expires = apply_filters( 'carrot_bunnycdn_incoom_plugin_' . self::OPTION_CACHE_GROUP . '_expires', DAY_IN_SECONDS, self::CACHE_KEY, $data );
	...
	protected function maybe_update_post_cache( $to_cache, $post_id = false ) {
		$post_id = carrot_bunnycdn_incoom_plugin_Utils::get_post_id( $post_id );
	...
	public static function merge_cache( $existing_cache, $merge_cache ) {
		if ( ! empty( $existing_cache ) ) {
			$post_cache_keys = array_map( 'carrot_bunnycdn_incoom_plugin_Utils::reduce_url', array_keys( $existing_cache ) );
			$existing_cache  = array_combine( $post_cache_keys, $existing_cache );
		}

		if ( ! empty( $merge_cache ) ) {
			$add_cache_keys  = array_map( 'carrot_bunnycdn_incoom_plugin_Utils::reduce_url', array_keys( $merge_cache ) );
