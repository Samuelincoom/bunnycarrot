 ...
 * @version 1.0.31
 */
class carrot_bunnycdn_incoom_plugin_Action_Queue implements carrot_bunnycdn_incoom_plugin_Queue_Interface {
	 ...
	 * @return carrot_bunnycdn_incoom_plugin_DateTime|null The date and time for the next occurrence, or null if there is no pending, scheduled action for the given hook.
	 */
	public function get_next( $hook, $args = null, $group = '' ) {
		...
		if ( $next_timestamp ) {
			return new carrot_bunnycdn_incoom_plugin_DateTime( "@{$next_timestamp}", new DateTimeZone( 'UTC' ) );
		}
