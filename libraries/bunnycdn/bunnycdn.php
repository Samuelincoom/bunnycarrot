...
class carrot_bunnycdn_incoom_plugin_BunnyCDN
{    
    ...
    public function listStorageZones()
    {
		...
		*/
		...
		$storages =  json_decode($api_call['data'], true);

		return array('status' => 'success', "storages" => $storages);
    }
	...
	public function Storage($api_key_storage='', $storageZoneRegion = 'de')
	{ 
		if(!$api_key_storage)
		{			 
	...
	{	
		...
		*/
		...
		$api_url = $this->fix_url($this->getStorageUrl().$storage_path );
	...
	public function PutFile($local_upload_file_path ='',  $storage_zone_path='', $storage_zone_file_path='' )
	{
		...
		*/
		...
		$key =  $this->api_key_storage;
		$api_url = $this->fix_url($this->getStorageUrl().$storage_file_path);
	...
	public function SecureLink($file_path='', $expiry_hr = 1)
	{
		$host_name = get_option('incoom_carrot_bunnycdn_incoom_plugin_aws_connection_bucket_base_url');
		$securityKey = get_option('incoom_carrot_bunnycdn_incoom_plugin_security_key');
		$path = $file_path;
