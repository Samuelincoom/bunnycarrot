	...
	private function save_action_to_db( ActionScheduler_Action $action, DateTime $date = null, $unique = false ) {
		...
		try {
			...
			if ( is_wp_error( $action_id ) ) {
				...
				throw new \RuntimeException( $wpdb->last_error ? $wpdb->last_error : __( 'Database error.','carrot-bunnycdn-incoom-plugin' ) );
			}
			...
			throw new \RuntimeException( sprintf( __( 'Error saving action: %s','carrot-bunnycdn-incoom-plugin' ), $e->getMessage() ), 0 );
		}
	}
...
",
	...
	protected function get_query_actions_sql( array $query, $select_or_count = 'select' ) {
		...
		if ( ! in_array( $select_or_count, array( 'select', 'count' ), true ) ) {
			throw new InvalidArgumentException( __( 'Invalid value for select or count parameter. Cannot query actions.','carrot-bunnycdn-incoom-plugin' ) );
		}
		...
		if ( ! is_null( $query['args'] ) ) {
			switch ( $query['partial_args_matching'] ) {
				case 'json':
					if ( ! $supports_json ) {
						throw new \RuntimeException( __( 'JSON partial matching not supported in your environment. Please check your MySQL/MariaDB version.','carrot-bunnycdn-incoom-plugin' ) );
					}
					...
					foreach ( $query['args'] as $key => $value ) {
						...
						if ( ! $placeholder ) {
							throw new \RuntimeException( sprintf(
								...
								__( 'The value type for the JSON partial matching is not supported. Must be either integer, boolean, double or string. %s type provided.','carrot-bunnycdn-incoom-plugin' ),
								$value_type
							) );
						}
				...
				case 'off':
					...
					break;
				default:
					throw new \RuntimeException( __( 'Unknown partial args matching value.','carrot-bunnycdn-incoom-plugin' ) );
			}
		}
	...
	public function cancel_action( $action_id ) {
		...
		if ( false === $updated ) {
			...
			throw new \InvalidArgumentException( sprintf( __( 'Unidentified action %s','carrot-bunnycdn-incoom-plugin' ), $action_id ) );
		}
	...
	public function delete_action( $action_id ) {
		...
		if ( empty( $deleted ) ) {
			throw new \InvalidArgumentException( sprintf( __( 'Unidentified action %s','carrot-bunnycdn-incoom-plugin' ), $action_id ) ); //phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
		}
	...
	protected function claim_actions( $claim_id, $limit, \DateTime $before_date = null, $hooks = array(), $group = '' ) {
		...
		if ( ! empty( $group ) ) {
			...
			if ( empty( $group_id ) ) {
				...
				throw new InvalidArgumentException( sprintf( __( 'The group "%s" does not exist.','carrot-bunnycdn-incoom-plugin' ), $group ) );
			}
		...
		if ( false === $rows_affected ) {
			throw new \RuntimeException( __( 'Unable to claim actions. Database error.','carrot-bunnycdn-incoom-plugin' ) );
		}
	...
	public function get_status( $action_id ) {
		...
		if ( null === $status ) {
			throw new \InvalidArgumentException( __( 'Invalid action ID. No status found.','carrot-bunnycdn-incoom-plugin' ) );
		} elseif ( empty( $status ) ) {
			throw new \RuntimeException( __( 'Unknown status found for action.','carrot-bunnycdn-incoom-plugin' ) );
		} else {
			return $status;
		}
	}
