...
abstract class ActionScheduler_Logger {
	...
	public function log_stored_action( $action_id ) {
		$this->log( $action_id, __( 'action created','carrot-bunnycdn-incoom-plugin' ) );
	}

	public function log_canceled_action( $action_id ) {
		$this->log( $action_id, __( 'action canceled','carrot-bunnycdn-incoom-plugin' ) );
	}

	public function log_started_action( $action_id, $context = '' ) {
		if ( ! empty( $context ) ) {
			/* translators: %s: context */
			$message = sprintf( __( 'action started via %s','carrot-bunnycdn-incoom-plugin' ), $context );
		} else {
			$message = __( 'action started','carrot-bunnycdn-incoom-plugin' );
		}
	...
	public function log_completed_action( $action_id, $action = NULL, $context = '' ) {
		if ( ! empty( $context ) ) {
			/* translators: %s: context */
			$message = sprintf( __( 'action complete via %s','carrot-bunnycdn-incoom-plugin' ), $context );
		} else {
			$message = __( 'action complete','carrot-bunnycdn-incoom-plugin' );
		}
		$this->log( $action_id, $message );
	}
	...
	public function log_failed_action( $action_id, Exception $exception, $context = '' ) {
		if ( ! empty( $context ) ) {
			...
			$message = sprintf( __( 'action failed via %1$s: %2$s','carrot-bunnycdn-incoom-plugin' ), $context, $exception->getMessage() );
		} else {
			/* translators: %s: exception message */
			$message = sprintf( __( 'action failed: %s','carrot-bunnycdn-incoom-plugin' ), $exception->getMessage() );
		}
		$this->log( $action_id, $message );
	}
	...
	public function log_timed_out_action( $action_id, $timeout ) {
		...
		$this->log( $action_id, sprintf( __( 'action marked as failed after %s seconds. Unknown error occurred. Check server, PHP and database error logs to diagnose cause.','carrot-bunnycdn-incoom-plugin' ), $timeout ) );
	}

	public function log_unexpected_shutdown( $action_id, $error ) {
		if ( ! empty( $error ) ) {
			/* translators: 1: error message 2: filename 3: line */
			$this->log( $action_id, sprintf( __( 'unexpected shutdown: PHP Fatal error %1$s in %2$s on line %3$s','carrot-bunnycdn-incoom-plugin' ), $error['message'], $error['file'], $error['line'] ) );
		}
	}

	public function log_reset_action( $action_id ) {
		$this->log( $action_id, __( 'action reset','carrot-bunnycdn-incoom-plugin' ) );
	}

	public function log_ignored_action( $action_id, $context = '' ) {
		if ( ! empty( $context ) ) {
			/* translators: %s: context */
			$message = sprintf( __( 'action ignored via %s','carrot-bunnycdn-incoom-plugin' ), $context );
		} else {
			$message = __( 'action ignored','carrot-bunnycdn-incoom-plugin' );
		}
		$this->log( $action_id, $message );
	}
	...
	public function log_failed_fetch_action( $action_id, Exception $exception = NULL ) {
		...
		if ( ! is_null( $exception ) ) {
			/* translators: %s: exception message */
			$log_message = sprintf( __( 'There was a failure fetching this action: %s','carrot-bunnycdn-incoom-plugin' ), $exception->getMessage() );
		} else {
			$log_message = __( 'There was a failure fetching this action','carrot-bunnycdn-incoom-plugin' );
		}
	...
	public function log_failed_schedule_next_instance( $action_id, Exception $exception ) {
		/* translators: %s: exception message */
		$this->log( $action_id, sprintf( __( 'There was a failure scheduling the next instance of this action: %s','carrot-bunnycdn-incoom-plugin' ), $exception->getMessage() ) );
	}
