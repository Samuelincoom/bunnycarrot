...
class ActionScheduler_ListTable extends ActionScheduler_Abstract_ListTable {
	...
	public function __construct( ActionScheduler_Store $store, ActionScheduler_Logger $logger, ActionScheduler_QueueRunner $runner ) {
		...
		$this->table_header = __( 'Scheduled Actions','carrot-bunnycdn-incoom-plugin' );

		$this->bulk_actions = array(
			'delete' => __( 'Delete','carrot-bunnycdn-incoom-plugin' ),
		);

		$this->columns = array(
			'hook'        => __( 'Hook','carrot-bunnycdn-incoom-plugin' ),
			'status'      => __( 'Status','carrot-bunnycdn-incoom-plugin' ),
			'args'        => __( 'Arguments','carrot-bunnycdn-incoom-plugin' ),
			'group'       => __( 'Group','carrot-bunnycdn-incoom-plugin' ),
			'recurrence'  => __( 'Recurrence','carrot-bunnycdn-incoom-plugin' ),
			'schedule'    => __( 'Scheduled Date','carrot-bunnycdn-incoom-plugin' ),
			'log_entries' => __( 'Log','carrot-bunnycdn-incoom-plugin' ),
		);
		...
		if ( empty( $request_status ) ) {
			...
			$this->columns  += array( 'claim_id' => __( 'Claim ID','carrot-bunnycdn-incoom-plugin' ) );
		...
		$this->row_actions = array(
			'hook' => array(
				'run' => array(
					'name'  => __( 'Run','carrot-bunnycdn-incoom-plugin' ),
					'desc'  => __( 'Process the action now as if it were run as part of a queue','carrot-bunnycdn-incoom-plugin' ),
				),
				'cancel' => array(
					'name'  => __( 'Cancel','carrot-bunnycdn-incoom-plugin' ),
					'desc'  => __( 'Cancel the action now to avoid it being run in future','carrot-bunnycdn-incoom-plugin' ),
					'class' => 'cancel trash',
				),
			),
		);
		...
		self::$time_periods = array(
			array(
				...
				'names'   => _n_noop( '%s year', '%s years','carrot-bunnycdn-incoom-plugin' ),
			),
			array(
				'seconds' => MONTH_IN_SECONDS,
				/* translators: %s: amount of time */
				'names'   => _n_noop( '%s month', '%s months','carrot-bunnycdn-incoom-plugin' ),
			),
			array(
				'seconds' => WEEK_IN_SECONDS,
				/* translators: %s: amount of time */
				'names'   => _n_noop( '%s week', '%s weeks','carrot-bunnycdn-incoom-plugin' ),
			),
			array(
				'seconds' => DAY_IN_SECONDS,
				/* translators: %s: amount of time */
				'names'   => _n_noop( '%s day', '%s days','carrot-bunnycdn-incoom-plugin' ),
			),
			array(
				'seconds' => HOUR_IN_SECONDS,
				/* translators: %s: amount of time */
				'names'   => _n_noop( '%s hour', '%s hours','carrot-bunnycdn-incoom-plugin' ),
			),
			array(
				'seconds' => MINUTE_IN_SECONDS,
				/* translators: %s: amount of time */
				'names'   => _n_noop( '%s minute', '%s minutes','carrot-bunnycdn-incoom-plugin' ),
			),
			array(
				'seconds' => 1,
				/* translators: %s: amount of time */
				'names'   => _n_noop( '%s second', '%s seconds','carrot-bunnycdn-incoom-plugin' ),
			),
		);
	...
	private static function human_interval( $interval, $periods_to_include = 2 ) {
		...
		if ( $interval <= 0 ) {
			return __( 'Now!','carrot-bunnycdn-incoom-plugin' );
		}
		...
		for ( $time_period_index = 0, $periods_included = 0, $seconds_remaining = $interval; $time_period_index < count( self::$time_periods ) && $seconds_remaining > 0 && $periods_included < $periods_to_include; $time_period_index++ ) {
			...
			if ( $periods_in_interval > 0 ) {
				...
				$output .= sprintf( _n( self::$time_periods[ $time_period_index ]['names'][0], self::$time_periods[ $time_period_index ]['names'][1], $periods_in_interval,'carrot-bunnycdn-incoom-plugin' ), $periods_in_interval );
	...
	protected function get_recurrence( $action ) {
		...
		if ( $schedule->is_recurring() ) {
			...
			if ( is_numeric( $recurrence ) ) {
				...
				return sprintf( __( 'Every %s','carrot-bunnycdn-incoom-plugin' ), self::human_interval( $recurrence ) );
			} else {
				return $recurrence;
			}
		}

		return __( 'Non-repeating','carrot-bunnycdn-incoom-plugin' );
	}
	...
	public function display_admin_notices() {
			...
			foreach ( $table_list as $table_name ) {
				if ( ! in_array( $wpdb->prefix . $table_name, $found_tables ) ) {
					$this->admin_notices[] = array(
						'class'   => 'error',
						'message' => __( 'It appears one or more database tables were missing. Attempting to re-create the missing table(s).' ,'carrot-bunnycdn-incoom-plugin' ),
					);
		...
		if ( $this->runner->has_maximum_concurrent_batches() ) {
			...
			if ( false === $async_request_lock_expiration || $async_request_lock_expiration < time() ) {
				...
				$async_request_message = sprintf( __( 'A new queue has begun processing. <a href="%s">View actions in-progress &raquo;</a>','carrot-bunnycdn-incoom-plugin' ), esc_url( $in_progress_url ) );
			} else {
				/* translators: %d: seconds */
				$async_request_message = sprintf( __( 'The next queue will begin processing in approximately %d seconds.','carrot-bunnycdn-incoom-plugin' ), $async_request_lock_expiration - time() );
			}
			...
			if ( 1 == $notification['success'] ) {
				...
				switch ( $notification['row_action_type'] ) {
					case 'run' :
						...
						$action_message_html = sprintf( __( 'Successfully executed action: %s','carrot-bunnycdn-incoom-plugin' ), $action_hook_html );
						break;
					case 'cancel' :
						/* translators: %s: action HTML */
						$action_message_html = sprintf( __( 'Successfully canceled action: %s','carrot-bunnycdn-incoom-plugin' ), $action_hook_html );
						break;
					default :
						/* translators: %s: action HTML */
						$action_message_html = sprintf( __( 'Successfully processed change for action: %s','carrot-bunnycdn-incoom-plugin' ), $action_hook_html );
						break;
				}
			} else {
				$class = 'error';
				/* translators: 1: action HTML 2: action ID 3: error message */
				$action_message_html = sprintf( __( 'Could not process change for action: "%1$s" (ID: %2$d). Error: %3$s','carrot-bunnycdn-incoom-plugin' ), $action_hook_html, esc_html( $notification['action_id'] ), esc_html( $notification['error_message'] ) );
			}
	...
	protected function get_schedule_display_string( ActionScheduler_Schedule $schedule ) {
		...
		if ( is_a( $schedule, 'ActionScheduler_NullSchedule' ) ) {
			return __( 'async','carrot-bunnycdn-incoom-plugin' );
		}
		...
		if ( gmdate( 'U' ) > $next_timestamp ) {
			...
			$schedule_display_string .= sprintf( __( ' (%s ago)','carrot-bunnycdn-incoom-plugin' ), self::human_interval( gmdate( 'U' ) - $next_timestamp ) );
		} else {
			/* translators: %s: date interval */
			$schedule_display_string .= sprintf( __( ' (%s)','carrot-bunnycdn-incoom-plugin' ), self::human_interval( $next_timestamp - gmdate( 'U' ) ) );
		}
	...
	protected function get_search_box_button_text() {
		return __( 'Search hook, args and claim ID','carrot-bunnycdn-incoom-plugin' );
	}
